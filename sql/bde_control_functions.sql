...
BEGIN
    ...
    IF (v_changed > 0) THEN
        ...
        WHERE 
            NOT EXISTS (
                SELECT id
                FROM   _tmp_inc_change
                WHERE  MISS.id = _tmp_inc_change.id
            );
...
BEGIN
    RETURN bde_ExecuteTemplate (
        $sql$
            ...
            FROM
                _tmp_inc_change CHG
                JOIN %1% ON (CHG.id = %1%.%3%)
                LEFT JOIN %2% ON (CHG.id = %2%.%3%)
            WHERE
                %2%.%3% IS NULL
        $sql$,
        ARRAY[
...
BEGIN
    RETURN bde_ExecuteTemplate (
        $sql$
            ...
            FROM
                _tmp_inc_change CHG
                LEFT JOIN %1% ON (CHG.id = %1%.%3%)
                JOIN %2% ON (CHG.id = %2%.%3%)
            WHERE
                %1%.%3% IS NULL
        $sql$,
        ARRAY[
...
BEGIN
    ...
    RETURN bde_ExecuteTemplate( $sql$
        UPDATE _tmp_inc_actions
        SET action='X'
        FROM %1% AS NEW_DAT,
             %2% AS CUR
        WHERE 
           _tmp_inc_actions.action = 'U' AND
           NEW_DAT.%3% = _tmp_inc_actions.id AND
           CUR.%3% = _tmp_inc_actions.id AND
           (%4%) <> (%5%)
        $sql$,
        ARRAY[
            p_inc_data_table::TEXT,
