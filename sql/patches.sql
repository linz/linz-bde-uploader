...
--------------------------------------------------------------------------------
-- LDS table all_linear_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS all_linear_parcels_pend CASCADE;

CREATE TABLE all_linear_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''all_linear_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE all_linear_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_all_line_par_pend_shape ON all_linear_parcels_pend USING gist (shape);

ALTER TABLE all_linear_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE all_linear_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE all_linear_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE all_linear_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table primary_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS primary_parcels_pend CASCADE;

CREATE TABLE primary_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''primary_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE primary_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_all_prim_par_pend_shape ON primary_parcels_pend USING gist (shape);

ALTER TABLE primary_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE primary_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE primary_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE primary_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table land_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS land_parcels_pend CASCADE;

CREATE TABLE land_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''land_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE land_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_lnd_par_pend_shape ON land_parcels_pend USING gist (shape);

ALTER TABLE land_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE land_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE land_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE land_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table hydro_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS hydro_parcels_pend CASCADE;

CREATE TABLE hydro_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''hydro_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE hydro_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_hyd_par_pend_shape ON hydro_parcels_pend USING gist (shape);

ALTER TABLE hydro_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE hydro_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE hydro_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE hydro_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table road_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS road_parcels_pend CASCADE;

CREATE TABLE road_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''road_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE road_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_road_par_pend_shape ON road_parcels_pend USING gist (shape);

ALTER TABLE road_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE road_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE road_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE road_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table non_primary_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS non_primary_parcels_pend CASCADE;

CREATE TABLE non_primary_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''non_primary_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE non_primary_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_non_prim_par_pend_shape ON non_primary_parcels_pend USING gist (shape);

ALTER TABLE non_primary_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE non_primary_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE non_primary_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE non_primary_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table non_primary_linear_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS non_primary_linear_parcels_pend CASCADE;

CREATE TABLE non_primary_linear_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''non_primary_linear_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE non_primary_linear_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_non_pril_par_pend_shape ON non_primary_linear_parcels_pend USING gist (shape);

ALTER TABLE non_primary_linear_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE non_primary_linear_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE non_primary_linear_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE non_primary_linear_parcels_pend TO bde_user;

--------------------------------------------------------------------------------
-- LDS table strata_parcels_pend
--------------------------------------------------------------------------------
DROP TABLE IF EXISTS strata_parcels_pend CASCADE;

CREATE TABLE strata_parcels_pend (
    id INTEGER NOT NULL,
    appellation VARCHAR(2048),
    affected_surveys VARCHAR(2048),
    parcel_intent VARCHAR(100) NOT NULL,
...
PERFORM AddGeometryColumn(''strata_parcels_pend'', ''shape'', 4167, ''GEOMETRY'', 2);

ALTER TABLE strata_parcels_pend ADD PRIMARY KEY (id);
CREATE INDEX shx_str_par_pend_shape ON strata_parcels_pend USING gist (shape);

ALTER TABLE strata_parcels_pend OWNER TO bde_dba;

REVOKE ALL ON TABLE strata_parcels_pend FROM PUBLIC;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE strata_parcels_pend TO bde_admin;
GRANT SELECT ON TABLE strata_parcels_pend TO bde_user;

-- only enable versioning if we already have versioned tables
IF EXISTS (SELECT table_version.ver_get_versioned_tables()) THEN
    PERFORM table_version.ver_enable_versioning(''lds'', ''all_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''all_linear_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''primary_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''land_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''hydro_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''road_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''non_primary_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''non_primary_linear_parcels_pend'');
    PERFORM table_version.ver_enable_versioning(''lds'', ''strata_parcels_pend'');
END IF;
