...
sub sendMessageType
{
    ...
    my $subject = $cfg->get($msgtype."subject","geo_bde log");
...
=head1 Syntax
  ...
  perl bde_geo.pl [options..] [tables..]
...
Select the configuration file that will be used.  Default is ~/config/bde_geo.cfg,
where ~ is the directory in which the bde_geo.pl script is located.
